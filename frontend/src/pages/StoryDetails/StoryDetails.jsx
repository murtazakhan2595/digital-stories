...
function StoryDetails() {
  ...
  const upVoteHandler = async () => {
    const data = { post: id, postedBy: story.postedBy };
  ...
  const postCommentHandler = async () => {
    const data = { text: newComment, story: id };
  ...
  const updateStoryHandler = async () => {
    navigate("/update-story", { state: { story } });
  };

  useEffect(() => {
    (async () => {
      const storyResponse = await getStoryById(id);
      const storyRes = storyResponse.data.data.story;
      ...
      const response = await getVoteStatus(id);
